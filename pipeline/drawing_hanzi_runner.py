"""绘画页汉字骨架 (hanzi_stroke / MakeMeAHanzi medians) → DrawingProcessPlanner → CRI。"""
...
from pipeline.text_pipeline import TEXT_SOURCE_HANZI_STROKE, build_text_pipeline
...
def run_hanzi_stroke_to_cri(
    ...
    *,
    ...
    output_dir: str | Path = "output/drawing_hanzi_run",
    user_lang: str = "zh",
    ...
    if extract_stats.get("ttf_fallback_used"):
        return DrawingTextRunResult(
            ok=False,
            error="hanzi_stroke 不允许 TTF fallback",
            output_dir=str(out),
        )
    ...
    path_cfg = PathConfig(mode="skeleton", skeleton_source="makemeahanzi")
    ...
    DebugExporter.write_strokes_preview(
        strokes_preview_src,
        raw_path,
        title="Raw Strokes Preview (MakeMeAHanzi medians)",
        canvas_w=map_w,
        canvas_h=map_h,
    )
    ...
    hanzi_summary = {
        ...
        "graphics_path": extract_stats.get("graphics_path", ""),
        "total_loaded_chars": extract_stats.get("total_loaded_chars", 0),
        "unique_required_chars": extract_stats.get("unique_required_chars", 0),
        "missing_chars": extract_stats.get("missing_chars", []),
        "glyph_count": extract_stats.get("glyph_count", 0),
        "stroke_count": extract_stats.get("strokes", 0),
    }
    drawing_summary = {
...
def make_run_output_dir(base_dir: str | Path, text: str) -> Path:
    ...
    return Path(base_dir) / f"{ts}_{safe}_hanzi_stroke"
